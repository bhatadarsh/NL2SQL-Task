...
A modular Natural Language to SQL analytics system that converts plain English questions into validated SQL queries, executes them on a relational database, and automatically generates analytical visualizations.

Unlike simple prompt-to-SQL demos, this project focuses on LLM system design with guardrails, validation layers, and structured reasoning.

Overview

Business users frequently need insights from databases but do not know SQL.
...
Instead of generating SQL directly, the system first extracts a structured intent representation using the LLM.
...
Using metadata from the schema, the system builds a join graph based on foreign keys.
...
If SQL validation fails, the system retries generation with stricter prompts.
...
These results are converted into a Pandas DataFrame for further processing.

9. Automatic Visualization

The system automatically determines the best visualization based on the data structure.
